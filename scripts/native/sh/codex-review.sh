...
# run can never hijack a Reviewer resume); with one it resumes that id verbatim. --resume-id <id> is
# the same thing spelled with a mandatory value, and it implies --resume. Every run - cold or
...
#   cat .aiwf/review-brief.txt | bash scripts/native/sh/codex-review.sh --project-root /repo --resume
set -euo pipefail
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --resume)
      ...
      # `--resume --class code` cannot silently swallow the class flag as a session id.
      RESUME=1
    ...
    --resume-id)
      ...
      # an empty --class degrading into the classless host.
...
# `last-review-session.txt` is written and read here and nowhere else, and the QA wrapper's own file
...
SESSION_STATE="$PROJECT_ROOT_ABS/$SCRATCH_REL/last-review-session.txt"
...
# pass would spend the operator's quota on the read-the-whole-repo pass they were avoiding.
...
clear_session_state() {
  ...
  if [ -n "$left" ]; then
    printf 'codex-review: ERROR: this run could not be identified in the codex session store AND the stale session id %s could NOT be removed from %s. Do not resume from it - pass --resume <id> explicitly.\n' "$left" "$SESSION_STATE" >&2
    return 0
  fi
  printf 'codex-review: this run could not be identified in the codex session store; no session id is recorded now (%s), so the next --resume will need an explicit id.\n' "$SESSION_STATE" >&2
}
...
record_session_id() {
  ...
  if [ -n "$id" ]; then
    ...
    printf '%s\n' "$id" > "$SESSION_STATE" 2>/dev/null \
      || printf 'codex-review: the session id could not be recorded in %s\n' "$SESSION_STATE" >&2
    return 0
  fi
  clear_session_state
}
