 ...
 *     (b) `newRender != upstream` the payload changed the artifact,
 *   because at update time a new render is a change the operator has never seen. Setup re-renders
 *   the same config; an update brings a different payload.
 *
...
/** Turns an accepted conflict resolution into a plan. No branch here overwrites anything silently. */
function applyResolution(ctx, { op, key, address, actual, newRender, renderHash, record }) {
  ...
  return {
    ...
    summary: isMerge ? `${key}: your merged content applied (upstream recorded, held)` : `${key}: the payload version applied`,
  };
}
...
export function assembleChanges({ from, to, pending, migrations, managedRegions }) {
  ...
  lines.push('the next update overwrites it, and deleting it costs nothing.', '');
  ...
  for (const entry of pending) {
    ...
    for (const op of ops) {
      ...
      else if (op.op === 'rerender-managed-region') lines.push(`- \`rerender-managed-region\` ${op.region ? `${op.file}#${op.region}` : op.file}`);
