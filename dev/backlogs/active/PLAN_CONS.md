...
| 3 | session id → resume | codex-review.ps1:86-93 (argv) → :106 (инвокация) → нов: tee+парс блок в същия файл → нов: `<scratchDir>/last-review-session.txt` → нов: -Resume клон до :86-93 → Решение 12 argv; огледално sh :116-123/:135 и qa :71-78/:91, :100-107/:119 | runtime |
| 4 | работещ bash | findBash :237-249 (ред + функционална проба) → BASH :250 → sh-секциите | test |
| 5 | доктрина пиннати | P-фразите → домовете → масивите :4437-4687 → findings :4785-4909 → CONTROLS :4956-5060 | test |
