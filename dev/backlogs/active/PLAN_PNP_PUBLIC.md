...
**Незатворено от мисията:** само издаването на 0.2.2. macOS leg-ът вече НЕ блокира push-а —
POSIX-005 го направи advisory (`continue-on-error`), а операторът обяви macOS официално
неподдържан/best-effort (реалният дефект — проследен кандидат, иска Mac). Затова push-ът на `main`
+ tag `v0.2.2` чака само собствената си дума + диалози. Всичко от 0.2.2 нататък (кодът и dev
записите) е локално, непушнато (`git rev-list --left-right --count origin/main...main` го брои в
момента на четене).
