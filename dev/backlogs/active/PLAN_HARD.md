...
**Consumer proof: зелен** (операторски relay, 2026-09-15) — публичният път
`/plugin marketplace update` → `/reload-plugins` → `/pnp:update` мина на консуматора. Relay-ът
дойде като „consumer proof green" без числа, и тук НЕ се измислят: за разлика от 0.2.3/0.2.4
записите този носи потвърждение без брой операции, конфликти, selfcheck сума и commit хеш оттам.
Ако операторът подаде изхода по-късно, числата се дописват на това място. **С това HARD-007 е
затворен изцяло и 0.2.5 е издадена и consumer-доказана.**
