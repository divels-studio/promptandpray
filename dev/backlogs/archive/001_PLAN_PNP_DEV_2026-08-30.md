...
### DEV-001 — DONE, commit `c355d38` (2026-08-30; main сесия, bootstrap изключение)
...
- Commit `c355d38` по операторска дума 2026-08-30. Следващ: DEV-002 (self-install) — чака дума.

### DEV-002 — DONE, commit `429262b` (2026-08-30; размразен и довършен по операторска дума)
...
### DEV-003 — DONE, Furnissimo commit `1e38923b` (2026-08-30; операторска сесия във Furnissimo + COO machine-local)
- Furnissimo (последната сесия с `--plugin-dir`): `/plugin marketplace add D:\promptandpray` →
  `/plugin install pnp@promptandpray` на project scope → рестарт БЕЗ `--plugin-dir`.
  `claude plugin list` показва `pnp@promptandpray` 0.1.0, project scope. `/pnp:selfcheck` без
  `--plugin-dir` (плъгинът от marketplace cache-а): 687/687, exit 0 — операторско отчитане от
  Furnissimo сесията, не измерено от тази.
- Furnissimo repo, един docs commit `1e38923b` („DEV-003 — Furnissimo consumes pnp@promptandpray
  from the local marketplace (project scope)"): `PLAN_PROMPTANDPRAY_0_1_1.md` махнат оттам
  (живее тук), CANDIDATES pointer към `D:\promptandpray\dev\backlogs\`, `PROJECT_OVERRIDES.md`
  §Workspace с консумацията + update пътя, `.claude/settings.json` с marketplace/enabledPlugins.
- Machine-local (операторска дума за destructive): worktree `D:\Furnissimo-pnp` + клонът му
  премахнати; memory копието `~/.claude/projects/D--Furnissimo-pnp/` изтрито; PNP memory-то
  живее в `~/.claude/projects/D--promptandpray/memory/` (`promptandpray-mission-status`,
  `new-ticket-needs-operator-word`, `use-agents-for-scans` + MEMORY.md).
- VERIFY, проверено от PNP сесията на 2026-08-30: `git -C D:/Furnissimo log --oneline -1` →
  `1e38923b`; `git -C D:/Furnissimo status --short` → празно; `git -C D:/Furnissimo worktree
  list` → без `-pnp` запис; `D:\Furnissimo-pnp` и `~/.claude/projects/D--Furnissimo-pnp` не
  съществуват. Тук: `aiwf-update.mjs --check` → up to date 0.1.0 (exit 0).
- Ревю: docs-class тикет, консумацията е операторска операция (Claude Code UI команди) — без
  Одитор пас; верификацията е първолична (горе). Нищо в payload-а не е пипано.

## Closeout
Мисията е изпълнена: `dev/` зона + self-install + Furnissimo като marketplace консуматор
(DEV-001 `c355d38`, DEV-002 `429262b`, DEV-003 Furnissimo `1e38923b`). Следващо по записания
ред: **P8** в `dev/backlogs/active/PLAN_PROMPTANDPRAY_0_1_1.md` — чака дума за диспач. Gate 2
off-plan доказателството (тих диспач с валиден ref) се записва при първия реален Writer диспач
на P8. Няма открит дълг.
